...
public class LogInEmailUseCase extends UseCaseBase<User, EmailParameters.Parameters> {

    private ISignInRepository iSignInRepository;
    private Scheduler schedulerThread;

    @Inject
    public LogInEmailUseCase(ISignInRepository iSignInRepository, Scheduler schedulerThread) {
    ...
    public Observable<User> implementUseCase(DisposableObserver observer, EmailParameters.Parameters parameters) {
        //Invocamos el método del repositorio responsable de hacer login mediante email.
        Observable<User> observable = iSignInRepository.logInWithEmail(parameters.getEmail(), parameters.getPassword());
